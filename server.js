...
const server = http.createServer((req, res) => {
    ...
    // ── GET /sw-config.js – Firebase config for the service worker ───────────
    // Service workers cannot use ES modules or dynamic import(), so we expose
    // the config as a plain self.* assignment that importScripts() can load.
    if(req.url.split("?")[0] === "/sw-config.js"){
        ...
        res.end(`self.firebaseSwConfig = ${JSON.stringify(cfg)};\n`);
        return;
