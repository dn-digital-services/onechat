...
**Firestore-only presence (no Realtime DB) on a multi-page static site:** there's no `onDisconnect()` equivalent in Firestore, so "online/offline" must be heartbeat-based: set `online:true`+`lastSeen` on load and every ~25s while visible, `online:false` on `visibilitychange`(hidden)/`beforeunload`/`pagehide`. Readers must also treat a stale `lastSeen` (no heartbeat for ~60s) as offline, since a crashed tab never fires the unload events and would otherwise show "online" forever.

**Firebase Storage `getDownloadURL()` bypasses security rules for that URL:** the returned URL embeds an access token that grants access to that object regardless of the Storage rules, by design (it's meant to be a shareable link). So a private-by-uid storage path (`users/{uid}/...`) can still have its file safely shown to *other* users once the URL itself is stored/shared via Firestore (e.g. in a chat message) — no rule changes needed for cross-user file/image sharing in a 1:1 chat.

**Firestore rules: block full-collection enumeration on an otherwise-open-read collection** by splitting `get` vs `list`: `allow get: if <broad condition>; allow list: if <broad condition> && request.query.limit <= 1;`. Lets any signed-in user fetch one known doc (e.g. watch a specific chat partner's profile) or run a targeted `where(...).limit(1)` lookup (e.g. phone-number search), while preventing `getDocs(collection(...))` from dumping every document (e.g. every user's phone number).
